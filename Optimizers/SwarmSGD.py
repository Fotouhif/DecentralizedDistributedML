...
class SwarmSGD(Optimizer):
	def __init__(self, params, kwargs, lr=0.01, momentum=0, dampening=0,
		...
		self.local_neigh = np.argwhere(np.asarray(self.pi_all) != 0.0).ravel() # Find connected agents
		self.pi = [self.pi_all[i] for i in self.local_neigh] # simplified pi -- with only connected agents
		self.step_counter = 0
	...
	def step(self, closure=None):
		...
		for group in self.param_groups:
			...
			for p in group['params']:
				...
				if self.step_counter%(self.I1+self.I2) >= self.I1:
